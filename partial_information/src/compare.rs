...
impl<T> PartialInformationFieldCompare for T where T: PartialInformationCompare {
    fn get_conflicts(field_path: String, expected: &Self, actual: &Self) -> Vec<String> {
        expected.get_conflicts(actual).into_iter()
...
impl<ItemT> PartialInformationFieldCompare for Option<ItemT> where ItemT: PartialInformationFieldCompare + Debug {
    fn get_conflicts(field_path: String, expected: &Self, actual: &Self) -> Vec<String> {
        match (expected, actual) {
            (None, None) => vec![],
            (None, Some(val)) => vec![format!("{} Expected null, but value was {:?}", field_path, val)],
            (Some(val), None) => vec![format!("{} Expected {:?}, but value was null", field_path, val)],
...
impl<ItemT> PartialInformationFieldCompare for Vec<ItemT> where ItemT: PartialInformationFieldCompare {
    fn get_conflicts(field_path: String, expected: &Self, actual: &Self) -> Vec<String> {
        if expected.len() != actual.len() {
            vec![format!("{}: Expected length {}, but length was {}", field_path, expected.len(), actual.len())];
        }

        Iterator::zip(expected.iter(), actual.iter())
            .enumerate()
            .map(|(i, (a, b))| {
