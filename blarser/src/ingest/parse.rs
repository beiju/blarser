...
use crate::events::{Event, EventAux, LetsGo};
use crate::state::StateInterface;

pub fn parse_feed_event(feed_event: EventuallyEvent, state: &StateInterface) -> QueryResult<(Event, Vec<(String, Uuid, EventAux)>)> {
    match feed_event.r#type {
        EventType::LetsGo => LetsGo::parse(feed_event),
        EventType::PlayBall => PlayBall::parse(feed_event),
        EventType::HalfInning => HalfInning::parse(feed_event),
